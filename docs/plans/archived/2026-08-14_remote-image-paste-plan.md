...
On send, the authenticated bridge accepts bounded binary image data, verifies its signature, resolves the target pane working directory through `pane.get`, writes a random file under `.hedr/uploads`, and then submits the text prompt with that path.
