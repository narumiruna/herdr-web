...
On send, the authenticated bridge accepts bounded binary image data, verifies its signature, resolves the target pane working directory through `pane.get`, writes a random file under `$HOME/.herdr-web/uploads`, and then submits the text prompt with that path.

Docker bind-mounts both the configurable project root and `$HOME/.herdr-web` at matching absolute paths so the container and host Agent see the same file.
