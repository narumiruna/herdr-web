...
- `$HOME/.herdr-web/runtime/` stores Docker helper PID files and logs.
...
- [x] Update `justfile` and `compose.yaml` to use `$HOME/.herdr-web/runtime/` and same-path Docker mounting; Just parsing, default and custom Compose rendering, and relative-path rejection pass.
