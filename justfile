...
up:
    ...
    port=$(docker compose port web 8080 | head -n 1 | sed 's/.*://')
    host=$(node scripts/lan-address.mjs)
    keep_proxy=1
...
down:
    ...
    set -eu
    docker compose down
    pid_file=.herdr-web-runtime/socket-proxy.pid
    if [ -f "$pid_file" ]; then
    ...
    rm -f .herdr-web-runtime/socket-proxy.log
    rmdir .herdr-web-runtime 2>/dev/null || true
